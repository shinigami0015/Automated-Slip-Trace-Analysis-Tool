...
class MainWindow(QMainWindow):
    ...
    def _test_matlab(self):
        try:
            ...
            msg.setStyleSheet(f"background: {get_theme_colors(self._is_dark)['BG_CARD']}; color: {get_theme_colors(self._is_dark)['TEXT_1']};")
            msg.setText("✅ <b>MATLAB Engine API for Python is properly installed and accessible!</b><br><br>The STRCRYST application is ready to run.")
            msg.setIcon(QMessageBox.Icon.Information)
            msg.exec()
        except ImportError:
            msg = QMessageBox(self)
